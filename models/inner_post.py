...
# 依貼文名稱頁數及筆數搜尋
def query_post_list_by_title(post_title,page_size,page_number):
    return[{'_id' : doc['_id'],
             'title': doc['title'],
             'time' : doc['time'],
             'tag' : doc['tag'],
             'score' : doc['score']}
           for doc in _db.INNER_POST_COLLECTION.find({'title':{'$regex':'.*'+ post_title +'.*'}}).skip(page_size * (page_number - 1)).limit(page_size)]

# 依貼文標籤篩選
def query_post_list_by_tag(tag_list,page_size,page_number):
    return[{'_id' : doc['_id'],
             'title': doc['title'],
             'time' : doc['time'],
             'tag' : doc['tag'],
